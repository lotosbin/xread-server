...
import {IDBArticle, TArticle} from "../service";
import {mongoConnectionString} from "../service";
import * as R from "ramda";

export async function nextParseSeriesArticle({before}: { before: string }): Promise<IDBArticle> {
...
export async function parseArticleSeries(article: TArticle): Promise<string> {
    return await parseSeries(article.title || "");
}

export async function parseSeries(title: string): Promise<string> {
    try {
